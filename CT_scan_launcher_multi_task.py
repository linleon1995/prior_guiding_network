...
MODEL_FLAG = {'zlevel_classify': True, 'rotate_module': True,
              'class_classify': True, 'crf': False}
CRF_CONFIG = {"g_sxy": 3, "g_compat": 3, "bi_sxy": 1, "bi_srgb": 110,
              "bi_compat": 3, "iterations": 10, "resolution": (32, 32)}
WIDTH = 256
HEIGHT = 256
HU_WINDOW = [-180, 250]
HU_WINDOW = [-125, 275]
...
parser.add_argument('--is_transform', type=bool, default=True,
                    help='')
...
def launch():
    ...
    net = unet_multi_task3.Build_Model(model_flag=MODEL_FLAG,
                                       nx=HEIGHT,
                                       ny=WIDTH,
    ...
    kwargs = {"learning_rate": FLAGS.learning_rate,
              "power": FLAGS.power, "epochs": FLAGS.epochs}
    path = FLAGS.output_path if FLAGS.restore else create_training_path(
        FLAGS.output_path)

    trainer = unet_multi_task3.Trainer(
        net, batch_size=FLAGS.batch_size, norm_grads=True, optimizer="adam", opt_kwargs=kwargs)
