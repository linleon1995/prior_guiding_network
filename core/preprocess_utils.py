...
def sample_voxel(voxel, num_samples, num_slices):
  """Convert HU value to pixel value
  Args:
    voxel: [H,W,C]
    num_samples:
    num_slices: C'
  Returns:
    sampled_voxel: [H,W,C']
  """
  sample_rate = tf.divide(num_slices, num_samples)
  sample_rate = tf.cast(sample_rate, tf.int32)
  indices = tf.expand_dims(tf.range(0, sample_rate, num_slices), axis=1)
  voxel = tf.transpose(voxel, [2,1,0])
  voxel = tf.gather_nd(voxel, indices)
  new_voxel = tf.transpose(voxel, [2,1,0])
  return new_voxel
  
def HU_to_pixelvalue(image, HU_window):
  """Convert HU value to pixel value
  Args:
    image:
    HU_window: a list for two HU values
  Returns:
    processed_image
  """
  image = tf.clip_by_value(image, clip_value_min=HU_window[0], clip_value_max=HU_window[1])
...
def scale_image_data(image, scale, scale_method=None):
  ...
  if image.get_shape().ndims == 3:
    ...
    raise ValueError("Only accept image sahpe [height,width,channel] or [batch,height,width,channel]")
  
  if scale_method is None:
    scale_method = tf.image.ResizeMethod.BILINEAR
    
  new_dim = tf.cast(
      tf.cast([height, width], tf.float32) * scale,
      tf.int32)
...
def resize_to_range(image,
  ...
  with tf.name_scope(scope, 'resize_to_range', [image]):
    ...
    image = tf.image.resize_bilinear(image, new_size, align_corners=align_corners)
    ...
    if label is not None:
      ...
      if label_layout_is_chw:
        ...
        # resized_label.set_shape([256,256,1])
...
if __name__=="__main__":
    # print(get_z_label("classification", num_slices=120, depth=119, z_class=50))
    # print(get_z_label("regression", num_slices=130, depth=0, z_class=50))
    a=np.ones([1,23,3])
    print(a.dtype)