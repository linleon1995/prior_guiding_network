...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 19 21:36:41 2019
...
def nearlist_sampler(img, x, y):
    ...
    w_max = tf.concat([wa, wb, wc, wd], axis=3)
    w_max = tf.argmax(w_max, axis=3)
    w_max = tf.one_hot(w_max, depth=4, on_value=1, off_value=0, axis=-1)
    ...
    wa = tf.cast(wa, 'float32')
    wb = tf.cast(wb, 'float32')
    wc = tf.cast(wc, 'float32')
    wd = tf.cast(wd, 'float32')
