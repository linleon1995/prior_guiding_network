...
EVAL_SCALES = [1.0]
HU_WINDOW = [-125, 275]
BIG_TEST = list(np.arange(300)) + [350, 481]
# TODO: if dir not exist. Don't build new one
IMG_LIST = [50, 60, 64, 70, 82, 222,226, 227, 228, 350, 481]
...
# CHECKPOINT = '/home/acm528_02/Jing_Siang/project/Tensorflow/tf_thesis/thesis_trained/run_000/model.ckpt-170000'
...
def main(unused_argv):
  ...
  with tf.Graph().as_default() as graph:
    ...
    # Add name to input and label nodes so we can add to summary.
    samples[common.IMAGE] = tf.identity(samples[common.IMAGE], name=common.IMAGE)
    if FLAGS.eval_split in ("train", "val"):
      samples[common.LABEL] = tf.identity(samples[common.LABEL], name=common.LABEL)
    ...
    if FLAGS.vis_guidance:      
      # TODO: correct guidance_dict, pred_dict
    ...
    for i in range(dataset.splits_to_sizes['2013_MICCAI_Abdominal'][FLAGS.eval_split]):
        ...
        if i in display_imgs:
            ...
            show_seg_results.display_figure(FLAGS.eval_split+'_pred_%04d' %i,
                                            [data[common.IMAGE][0,...,0], pred[0], pred[0]],
            ...
            # show_seg_results.set_axis_off()
            # show_seg_results.display_figure(FLAGS.eval_split+'_aliasing_test_%04d' %i,
        ...
        if FLAGS.vis_guidance:
          ...
          if i in display_imgs:
            ...
            layers, pred_layers = sess.run([guidance_dict,
                                            pred_dict], feed_dict=_feed_dict)
        ...
        if FLAGS.vis_features:
          if i in display_imgs:
            ...
            for cc in range(0, 32, 4):
              for w in range(5):
                filename = "{}-feature1-sample{}-stage{}-feature{}".format(FLAGS.eval_split, i, w+1, cc)
                ...
                filename = "{}-feature2-sample{}-stage{}-feature{}".format(FLAGS.eval_split, i, w+1, cc)
