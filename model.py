...
def pgb_network(images, 
  ...
  with tf.variable_scope(scope, 'pgb_network', [images]) as sc:
#    features, layers_dict = plain_encoder(images, is_training=is_training)
    ...
    layers_dict = {"low_level1": end_points["pgb_network/resnet_v1_50/conv1_3"],
                    ...
                    "low_level4": end_points["pgb_network/resnet_v1_50/block4"]}
    if not is_training:
      layers_dict["resnet_feature"] = features
      
    if prior_imgs is not None or prior_segs is not None:
      # TODO: z_classes parameter
      prior_img, prior_seg, z_pred = get_prior(features, 
...
def get_prior(features, batch_size, num_classes, num_slices, prior_slices, z_classes, prior_imgs, prior_segs, z_label_method, scope=None):
  ...
  with tf.variable_scope(scope, 'prior_network') as sc:
    if z_label_method == 'regression':
      ...
      # indices = tf.cast(tf.multiply(tf.cast(num_slices, tf.float32), z_pred), tf.int32)
      
      # import numpy as np
      # data = np.reshape(np.arange(2000), [5,4,10,10])
      # x = tf.constant(data)
      # ii = tf.stack([0,1,2,3,4], axis=0)
      # ii = tf.expand_dims(ii, axis=1)
      # # ii = tf.tile(ii,[1,2])
      # result = tf.gather_nd(x, ii)
      # # r = tf.transpose(result, [3,1,2,0])
